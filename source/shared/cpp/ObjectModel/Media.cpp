    ...
    std::vector<std::shared_ptr<AdaptiveCardParseWarning>>& warnings, const Json::Value& json)
{
    ...
    auto sources = ParseUtil::GetElementCollectionOfSingleType<MediaSource>(elementParserRegistration,
        actionParserRegistration, warnings, json, AdaptiveCardSchemaKey::Sources, MediaSourceParser::Deserialize, true);
    ...
    for (auto source : sources)
    {
        ...
        {
            warnings.push_back(std::make_shared<AdaptiveCardParseWarning>(AdaptiveSharedNamespace::WarningStatusCode::InvalidMediaMix,
...
{
    return MediaParser::Deserialize(
