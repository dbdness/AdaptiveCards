...
std::string ValidateColor(const std::string& backgroundColor, std::vector<std::shared_ptr<AdaptiveCardParseWarning>>& warnings)
{
    ...
    if (!isValidColor)
    {
        warnings.emplace_back(std::make_shared<AdaptiveCardParseWarning>(AdaptiveSharedNamespace::WarningStatusCode::InvalidColorFormat,
    ...
    if (std::regex_search(requestedDimension, matches, pattern))
    {
        ...
        catch (const std::invalid_argument&)
        {
            warnings.emplace_back(std::make_shared<AdaptiveCardParseWarning>(
                AdaptiveSharedNamespace::WarningStatusCode::InvalidDimensionSpecified, warningMessage + requestedDimension));
        }
        catch (const std::out_of_range&)
        {
            warnings.emplace_back(std::make_shared<AdaptiveCardParseWarning>(
                AdaptiveSharedNamespace::WarningStatusCode::InvalidDimensionSpecified, "out of range: " + requestedDimension));
        }
    }
    else
    {
